...
import { isPureEnglishSegment } from '../lib/englishCandidateFilter'
import {
  ...
  listDeepSeekEnglishOnlySegments,
  validateDeepSeekAlignmentRows,
  buildAlignmentValidationWarnings
} from '../lib/realAlignmentBatch'
import type { SettingsState } from '../types'
import type { AlignmentPreviewDebug } from '../store/alignmentPreviewStore'
...
function buildDebugPayload(
  base: Omit<
    AlignmentPreviewDebug,
    'poolSegmentTotal' | 'pureEnglishPoolTotal' | 'deepSeekCandidateSent' | 'excludedFromDeepSeekCount' | 'validationWarnings'
  > & {
  ...
  return {
    ...
    parsedJson: base.parsedJson,
    latencyMs: base.latencyMs,
    usagePromptTokens: base.usagePromptTokens,
    poolSegmentTotal: base.poolSegmentTotal ?? null,
    pureEnglishPoolTotal: base.pureEnglishPoolTotal ?? null,
    deepSeekCandidateSent: base.deepSeekCandidateSent ?? null,
    excludedFromDeepSeekCount: base.excludedFromDeepSeekCount ?? null,
...
export function RealAlignmentSandbox({ settings }: { settings: SettingsState }): JSX.Element {
  ...
  const canRun =
    subtitles.length > 0 && listDeepSeekEnglishOnlySegments(segments).length > 0 && phase !== 'loading'
  ...
  const runBatch = useCallback(async () => {
    ...
    const poolSegmentTotal = segments.length
    const pureEnglishPoolTotal = listDeepSeekEnglishOnlySegments(segments).length
    const excludedFromDeepSeekCount = segments.filter(
      (s) => s.text.trim().length > 0 && !(s.language === 'english' && isPureEnglishSegment(s.text))
    ).length
    ...
    if (windowSegs.length === 0) {
      window.alert('没有符合「英文标签 + 纯英文内容」的 Script Pool 片段。请检查英文稿或过滤含中文的行。')
      return
    }
    ...
    try {
      ...
      if (!result.ok) {
        setRunError(
          ...
          buildDebugPayload({
            ...
            poolSegmentTotal,
            pureEnglishPoolTotal,
            deepSeekCandidateSent: windowSegs.length,
            excludedFromDeepSeekCount,
            validationWarnings: []
          })
        )
        return
      ...
      const allowed = new Set(windowSegs.map((s) => s.id))
      const validated = validateDeepSeekAlignmentRows(parsed.data.matches, allowed)
      const validationWarnings = buildAlignmentValidationWarnings(validated)
      setSuccess({
        matches: validated,
        ...
        debug: buildDebugPayload({
          ...
          rawResponse: result.rawText,
          parseError: null,
          parsedJson: JSON.stringify({ matches: validated }, null, 2),
          latencyMs: result.latencyMs,
          usagePromptTokens: usagePt,
          poolSegmentTotal,
          pureEnglishPoolTotal,
  ...
  const applyableCount = previewMatches?.filter((m) => m.applyable).length ?? 0

  const handleApply = useCallback(() => {
    if (!previewMatches || previewMatches.length === 0) return
    const applyable = previewMatches
      .filter((m) => m.applyable)
      .map(({ validationFlags: _f, applyable: _a, ...row }) => row)
    if (applyable.length === 0) {
      window.alert('当前预览中没有可应用的结果（可能全部为 invalid_candidate 或 invalid_segment_id）。')
      return
    }
    ...
    <div className="mt-4 space-y-3 border-t border-[var(--color-border-subtle)] pt-4">
      <div>
        ...
        <p className="type-caption mt-1 leading-snug text-secondary">
          使用 DeepSeek 对当前批次 <span className="text-primary font-medium">5</span> 条中文字幕与附近
          <span className="text-primary font-medium">纯英文</span>
          Script Pool 片段对齐（仅 <span className="font-medium">language=english</span> 且文本通过纯英文检测；结果先预览）。
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button
          ...
          disabled={phase !== 'ready' || applyableCount === 0}
          onClick={handleApply}
        >
          Apply AI Result
        </button>
        ...
        <div className="rounded-lg border border-[var(--color-border-subtle)] bg-[var(--color-bg-elevated)] p-2.5">
          ...
          {applyableCount === 0 ? (
            <p className="type-caption text-amber-600 dark:text-amber-400">
              无可应用项：请检查 invalid_candidate / invalid_segment_id 标记，或重新运行对齐。
            </p>
          ) : null}
          <div className="max-h-48 space-y-2 overflow-y-auto text-[12px] leading-snug">
            {previewMatches.map((m, i) => (
              <div
                key={`${m.subtitleId}-${i}`}
                className={`rounded border border-[var(--color-border-subtle)] bg-[var(--color-bg-base)] p-2${m.applyable ? '' : ' opacity-80'}`}
              >
                <p className="font-mono tabular-nums text-meta">#{String(m.subtitleId).padStart(3, '0')}</p>
                {!m.applyable && m.validationFlags.length > 0 ? (
                  <p className="type-caption mt-1 text-amber-600 dark:text-amber-400">
                    {m.validationFlags.join(', ')}
                  </p>
                ) : null}
      ...
      <div>
        ...
        {debugOpen && (debug || phase === 'error') ? (
          <div className="mt-2 space-y-2 rounded-lg border border-dashed border-[var(--color-border-subtle)] p-2 font-mono text-[11px] leading-relaxed text-secondary">
            {debug ? (
              <>
                {debug.poolSegmentTotal != null ? <p>script segments (raw): {debug.poolSegmentTotal}</p> : null}
                {debug.pureEnglishPoolTotal != null ? (
                  <p>English candidates after filter (full pool): {debug.pureEnglishPoolTotal}</p>
                ) : null}
                {debug.deepSeekCandidateSent != null ? (
                  <p>English candidates sent (window): {debug.deepSeekCandidateSent}</p>
                ) : null}
                {debug.excludedFromDeepSeekCount != null ? (
                  <p>excluded (non-pool / non-pure-EN / etc.): {debug.excludedFromDeepSeekCount}</p>
                ) : null}
                {debug.validationWarnings.length > 0 ? (
                  <div className="rounded border border-amber-500/40 bg-amber-500/10 p-2 text-amber-800 dark:text-amber-200">
                    <p className="font-semibold">validation warnings</p>
                    <ul className="mt-1 list-inside list-disc">
                      {debug.validationWarnings.map((w) => (
                        <li key={w}>{w}</li>
                      ))}
                    </ul>
                  </div>
